...
impl Clock {
    ...
    pub fn add_minutes(&mut self, minutes: i32) -> Self {
        if minutes.is_negative() {
            let hours_to_subtract = (minutes / 60).abs();
            let minutes_to_subtract = minutes.abs() % 60;
            self.hours -= hours_to_subtract;
            self.minutes -= minutes_to_subtract;
            if self.minutes.is_negative() {
                self.minutes = 60 + self.minutes;
                self.hours -= 1;
            }
            while self.hours.is_negative() {
        ...
        if new_minutes < 60 {
            if new_minutes.is_negative() {
                self.hours -= 1;
                if self.hours.is_negative() {
                    self.hours = 24 + self.hours;
                }
                self.minutes = 0 + new_minutes;
                if self.minutes.is_negative() {
                    self.minutes = 60 + self.minutes;
                }
            } else {
            ...
            let new_hours: i32 = new_minutes / 60;
    ...
    fn fix_offsets(mut self) -> Self {
        self.add_minutes(0)
            .fix_negative_minutes_offset()
            .fix_negative_hour_offset()
            .fix_sixty_mins()
    }

    fn fix_sixty_mins(mut self) -> Self {
        if self.minutes == 60 {
            self.hours += 1;
            self.minutes = 0;
        }
    ...
    fn fix_negative_minutes_offset(mut self) -> Self {
        if self.minutes.is_negative() {
            let offset = self.minutes % 60;
            self.hours -= (self.minutes / 60).abs() + 1;
            self.minutes = 60 - offset.abs();
        }
        return self;
    }

    fn fix_negative_hour_offset(mut self) -> Self {
        if self.hours.is_negative() {
            let offset = self.hours % 24;
            self.hours = 24 - offset.abs();
        }
        return self;
    }
    // To string example
    // fn to_string(&self) -> String {
    //     format!("Its {} hours and {} minutes", self.hours, self.minutes)
    // }
}

impl fmt::Display for Clock {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let formatted_hours = if self.hours < 10 {
            format!("0{}", self.hours)
        } else {
            format!("{}", self.hours)
        };

        let formatted_minutes = if self.minutes < 10 {
...
fn main() {
    ...
    let mut clock = Clock::new(2, 20);
    println!("My Clock BEFORE: {:?}", clock);

    clock.add_minutes(-3000);

    println!("My Clock AFTER: {}", clock)
}
